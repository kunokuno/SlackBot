...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 var inCharge;
 if(matchData[0]){
    inCharge = matchData[0].name;
 }else{
    inCharge = message[0].match[1];
 }
...
//リマインド用 ゴミ捨ての予定があるか確認
 function checkGarbageEvents(auth, bot, remindId){
    ...
    if (response.items.length === 0) {
        ...
        var eventStart = events[0].start.dateTime;//日付表記 用調整
        ...
        var matchData = nameList.member.filter(function(item,index){
                            if(item.name == events[0].summary){return true;}
                        });

        var botText;
        if(matchData[0]){
          botText = matchData.slackId;
        }else{
          botText = events[0].summary;
        }

        bot.say({
            text: '【ゴミ捨て】担当者:'+botText,
            channel: remindId
        });
    }
  });
